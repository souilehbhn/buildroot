...
define AMD_BUILD_CMDS
	( cd $(@D)/AMD && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
...
define CAMD_BUILD_CMDS
	( cd $(@D)/CAMD && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
...
define CCOLAMD_BUILD_CMDS
	( cd $(@D)/CCOLAMD && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
define AMD_INSTALL_STAGING_CMDS
...
define COLAMD_BUILD_CMDS
	( cd $(@D)/COLAMD && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
...
define CHOLMOD_BUILD_CMDS
	( cd $(@D)/CHOLMOD && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
define CHOLMOD_INSTALL_STAGING_CMDS
	( cd $(@D)/CHOLMOD && $(MAKE) $(TARGET_CONFIGURE_OPTS) install INSTALL=$(STAGING_DIR)/usr/ )
endef
define CHOLMOD_INSTALL_TARGET_CMDS
	( cd $(@D)/CHOLMOD && $(MAKE) $(TARGET_CONFIGURE_OPTS) install INSTALL=$(TARGET_DIR)/usr/ )
endef
endif
ifeq ($(BR2_PACKAGE_SUITESPARSE_CXSPARSE),y)

define CXSPARSE_BUILD_CMDS
	( cd $(@D)/CXSparse && $(MAKE) $(TARGET_CONFIGURE_OPTS) library )
endef
define CXPSARSE_INSTALL_STAGING_CMDS
